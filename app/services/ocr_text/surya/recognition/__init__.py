...
from app.services.ocr_text.surya.common.util import mark_step
...
from app.services.ocr_text.surya.input.processing import convert_if_not_rgb, slice_polys_from_image, slice_bboxes_from_image, slice_polys_from_image_v2
from app.services.ocr_text.surya.recognition.loader import RecognitionModelLoader
from app.services.ocr_text.surya.recognition.postprocessing import truncate_repetitions
from app.services.ocr_text.surya.recognition.processor import SuryaProcessor
from app.services.ocr_text.surya.recognition.util import sort_text_lines
from app.services.ocr_text.surya.recognition.schema import TextLine, OCRResult
from app.services.ocr_text.surya.recognition.process_bbox import process_bbox_detect
from app.services.ocr_text.surya.recognition.model.model_paddle import perform_ocr
...
class RecognitionPredictor(BasePredictor):
    ...
    def __call__(
            ...
            table_predictions: List[dict] | None = None,
            ...
            rec_paddles, confidence_paddles = self.recognition_ocr(
                flat['slice_crop'],
                perform_ocr,
                "PaddleOCR"
            )
            ...
            if source == "KONDOR":
                rec_predictions, confidence_scores = self.analysis_model(rec_predictions, confidence_scores, rec_paddles, confidence_paddles)
    ...
    def detect_and_slice_bboxes(
        ...
        if table_predictions:
            for bbox_cell in table_predictions:
                list_cells = bbox_cell['cells']

            
            for idx, det_pred in enumerate(det_predictions):
                new_det_predictions = process_bbox_detect(det_pred.bboxes, list_cells)

            for idx, (det_pred, image, highres_image, lang) in enumerate(zip([new_det_predictions], images, highres_images, langs)):
                polygons = [p['polygon'] for p in det_pred]
                if highres_image:
                    ...
                    slices, slices_crop = slice_polys_from_image_v2(highres_image, scaled_polygons)
                else:
        ...
        return {
            ...
            "slice_crop": all_slices_crop
        }

    def extract_number(self, id_number, pattern):
        cleaned = re.sub(r"[\.\,\|]", "", id_number)
        match = re.search(pattern, cleaned)
        return bool(match)
    
    def extract_gtcg(self, id_number, pattern):
        match = re.search(pattern, id_number)
        return bool(match)

    def analysis_model(self, texts_model_1, confidences_model_1, texts_model_2, confidences_model_2):
        texts_all_models = [texts_model_1]
        confidences_all_models = [confidences_model_1]
        ...
        for i in range(num_indices):
            ...
            if self.extract_number(texts_model_2[i], r"^(?:[A-Z0-9]+-[0-9]+|\d{1,3}(?:\.\d{3})+|\d{6,})$"):
                final_texts.append(texts_model_2[i])
                final_scores.append(confidences_model_2[i])
                continue
    ...
    def recognition_ocr(self, images, model_ocr, name):
        texts = []
        scores = []

        for image in tqdm(images, desc=f"Recognition with {name} model"):
